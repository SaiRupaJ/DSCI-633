...
class my_GA:
        ...
        if decision not in self.evaluated:
            ...
            for fold in range(self.crossval_fold):
                ...
                try:
                    ...
                    pred_proba = None
                actuals = y_test
                objs = np.array(self.obj_func(predictions.tolist(), actuals.tolist(), pred_proba.tolist()))
    ...
    def is_better(self, a, b):
        ...
        if all(obj_a[i] >= obj_b[i] for i in range(len(obj_a))):
        ...
        for i, x in enumerate(self.generation):
            ...
            for j in range(len(x)):
                if np.random.random() < self.mutation_rate:
                    ...
                    if isinstance(boundary[0], (int, float)) and isinstance(boundary[1], (int, float)):
                        val = np.random.random() * (float(boundary[1]) - float(boundary[0])) + float(boundary[0])
